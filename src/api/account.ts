...
export function AccountLogin (data: Record<string, any>) {
    return request({
        url: 'http://39.100.72.181:9000/oauth2/token',
        method: 'post',
        headers:{Authorization: 'Basic MzIxOjI0ZGVlM2RlYjc3NDQ4ZGNiZTliZTk1Mzc3NGFiYWJl','Content-Type':'application/x-www-form-urlencoded'},
...
export function logout (token:string,type:number = 1) {
    return request({
        url: 'http://39.100.72.181:9000/oauth2/revoke',
        method: 'post',
